...
export async function POST(_req: Request, ctx: { params: Promise<{ orderId: string }> }) {
  ...
  const resp = await fetch(
    `${BACKEND_BASE_URL}/api/admin/orders/${encodeURIComponent(orderId)}/approve`,
    {
      method: "POST",
      headers: {
  ...
  const data: unknown = text && isJson ? safeParseJson(text) : null;

  if (!resp.ok) {
    const trimmed = text.trim();
    const plainTextMessage = trimmed && !trimmed.startsWith("<") ? trimmed : null;
    const message =
      extractMessage(data) ??
      plainTextMessage ??
      (resp.status === 401
        ? "Backend unauthorized (token invalid/expired). Please logout and login again."
