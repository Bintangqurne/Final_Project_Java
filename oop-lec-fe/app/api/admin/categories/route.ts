...
export async function GET(req: Request) {
  ...
  const resp = await fetch(
    `${BACKEND_BASE_URL}/api/admin/categories?page=${encodeURIComponent(page)}&size=${encodeURIComponent(size)}`,
    {
      headers: {
...
export async function POST(req: Request) {
  ...
  const resp = await fetch(`${BACKEND_BASE_URL}/api/admin/categories`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
