...
export async function PATCH(req: Request, ctx: { params: Promise<{ cartItemId: string }> }) {
  ...
  const resp = await fetch(`${BACKEND_BASE_URL}/api/cart/items/${encodeURIComponent(cartItemId)}`, {
    method: "PATCH",
    headers: {
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ cartItemId: string }> }) {
  const { cartItemId } = await ctx.params;
